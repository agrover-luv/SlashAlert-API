...
namespace SlashAlert.Repositories.Adapters.MongoDb
{
    public class MongoDbPriceHistoryRepository : BaseMongoDbRepository<PriceHistory>, IPriceHistoryRepository
    {
        ...
        public async Task<IEnumerable<PriceHistory>> GetByProductIdAsync(string productId, string createdBy)
        {
            var filter = Builders<PriceHistory>.Filter.And(
                Builders<PriceHistory>.Filter.Eq(x => x.ProductId, productId),
                Builders<PriceHistory>.Filter.Eq(x => x.CreatedBy, createdBy)
            );
        ...
        public async Task<IEnumerable<PriceHistory>> GetByDateRangeAsync(DateTime startDate, DateTime endDate, string createdBy)
        {
        ...
        public async Task<IEnumerable<PriceHistory>> GetPriceDropsAsync(string createdBy)
        {
        ...
        public async Task<PriceHistory?> GetLatestPriceAsync(string productId, string createdBy)
        {
            var filter = Builders<PriceHistory>.Filter.And(
                Builders<PriceHistory>.Filter.Eq(x => x.ProductId, productId),
                Builders<PriceHistory>.Filter.Eq(x => x.CreatedBy, createdBy)
